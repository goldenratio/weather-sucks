  ...
  return value === undefined
    ...
    : html`
        <div class="temperature">
          <div class="weather-digit">
            ${toLocaleString(value)}<sup><span class="sup">° ${unit}</span></sup>
          </div>
        </div>
      `;
};
...
export const DoesItSuck = ({ value, blur }) => {
  ...
  return html`
    <h1 class=${`does-it-suck ${blurClass}`}>${value ? 'Yes, It Sucks!' : ''}</h1>
  `;
};
...
export const SettingsIcon = ({ onClick }) => {
  return html`
    ...
    >
      <svg class="settings-icon" height="48" width="48" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
